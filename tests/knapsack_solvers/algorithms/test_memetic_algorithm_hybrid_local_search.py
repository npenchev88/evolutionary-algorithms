...
def test_memetic_algorithm_hybrid_local_search_returns_valid_result(complex_data):
    np.random.seed(42)
    # random.seed(None)
    # np.random.seed(None)
    weights, values, max_weight = complex_data
    ma = MemeticAlgorithmHybridLocalSearch(weights, values, max_weight=50)
    result = ma.solve(timeout=2)
    assert isinstance(result, list)
    assert result[0] == "MEMETICALGORITHMHYBRIDLOCALSEARCH"
